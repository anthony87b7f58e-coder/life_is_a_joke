...
class EnhancedMultiIndicatorStrategy(BaseStrategy):
    ...
    def __init__(self, config, client, database, risk_manager):
        """Initialize enhanced multi-indicator strategy"""
        ...
        # Minimum score to generate signal (out of 100)
    ...
    def _analyze_symbol(self, symbol: str) -> List[Dict]:
        ...
        # Get klines data (15-minute candles for more signals)
    ...
    def _calculate_macd(self, prices: List[float]) -> Tuple[float, float, float]:
        ...
        # Simple approximation for signal line (should use EMA of MACD)
    ...
    def _calculate_buy_score(self, price: float, ema_fast: float, ema_medium: float,
        ...
        # EMA alignment (bullish) - 25 points
        ...
        # Bollinger Bands bounce - 15 points
        if price < bb_lower:
            score += 15
    ...
    def _calculate_sell_score(self, price: float, ema_fast: float, ema_medium: float,
        ...
        # EMA alignment (bearish) - 25 points
        if ema_fast < ema_medium < ema_slow:
            score += 25
        elif ema_fast < ema_medium:
            score += 15
        ...
        # Bollinger Bands resistance - 15 points
        if price > bb_upper:
            score += 15
        elif price > bb_middle:
            score += 8
        
        # Price below key EMAs - 20 points
        if price < ema_fast:
            score += 20
