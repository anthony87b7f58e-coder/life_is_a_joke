...
print_status "Navigating to repository directory: $REPO_DIR"
if [ ! -d "$REPO_DIR" ]; then
    print_error "Repository directory not found: $REPO_DIR"
    print_error "Please clone the repository first:"
    print_error "  mkdir -p ~/trading-bot-setup"
    print_error "  cd ~/trading-bot-setup"
    print_error "  git clone <repository-url> life_is_a_joke"
    exit 1
fi
